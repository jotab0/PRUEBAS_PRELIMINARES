...
// SUPERBLOQUE - bloques.dat

static void __inicializar_superbloque (void *module, t_config *tempSuperbloque){
    superbloque = (t_superbloque *) module;

    superbloque->BLOQUE_INICIAL = (uint32_t) config_get_int_value(tempSuperbloque, "BLOQUE_INICIAL");
    superbloque->TAMANIO_ARCHIVO = (uint32_t) config_get_int_value(tempSuperbloque, "TAMANIO_ARCHIVO");

    return;
}

t_superbloque *crear_superbloque (char *pathSuperbloque){
    t_superbloque *fcb = malloc(sizeof(*superbloque));

    // Checkear que se haya inicializado bien
    int32_t inicializacionCorrecta = config_init(superbloque, pathSuperbloque, es_logger, __inicializar_superbloque);
